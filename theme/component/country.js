(function ($) {
    $.fn.county = function (options) {
        ...
        return this.each(function () {
            ...
            container.append('<span class="county-days-wrapper first"><span class="county-days">00</span></span><span class="county-hours-wrapper separator-left border-separator-right"><span class="county-hours">00</span></span><span class="county-minutes-wrapper separator-left separator-left"><span class="county-minutes">00</span></span><span class="county-seconds-wrapper separator-left last"><span class="county-seconds">00</span></span><div class="clearfix"></div><span class="county-label-days">gün</span><span class="county-label-hours">saat</span><span class="county-label-minutes">dak</span><span class="county-label-seconds">san</span>');
            if (container.attr('id') == undefined || container.attr('id') == null) {
                container.attr('id', Math.random());
            }
            var w = 0;
            container.find('>span').each(function () {
                if ($(this).hasClass('county-days-wrapper') || $(this).hasClass('county-hours-wrapper') || $(this).hasClass('county-minutes-wrapper') || $(this).hasClass('county-seconds-wrapper')){
                    w = $(this).outerWidth();
				}
            });
            ...
            var reflectionContainer = container.first().clone(true).css({ opacity: settings.reflectionOpacity }).attr('id', container.attr('id') + '-refl').addClass('county-reflection');
            ...
            function aimateObject(element, reflectionElement, oldValue, newValue, type) {
                if (type == 'fade') {
                    ...
                    var copy = element.first().clone(true);
					
 				    var reflectionCopy = null;

                    if (settings.reflection){
                        reflectionCopy = reflectionElement.first().clone(true);
					}
                    ...
                    copy.animate({ marginTop: 0 }, settings.speed, function (e) { });
					...
					var elementNumber = element.length;
					if (elementNumber > 2) {
					  element.remove().end();
					}
                }
...
//
                return { 
				  days: formatNumber(Math.max(0, days), false), 
